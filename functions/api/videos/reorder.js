...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await env.NAV_DB.batch(items.map(item => env.NAV_DB.prepare(
      'UPDATE videos SET sort_order = ?, update_time = CURRENT_TIMESTAMP WHERE id = ?'
    ).bind(Number(item.sort_order), Number(item.id))));

    return jsonResponse({ code: 200, message: '视频排序已保存' });
  } catch (e) {
    return errorResponse(`保存视频排序失败: ${e.message}`, 500);
  }
}
